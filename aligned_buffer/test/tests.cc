...
struct RAIISignaler {
  ...
  bool constructed() const noexcept {
    return parameterized1_constructor or parameterized2_constructor or default_constructor or copy_constructor
        or move_constructor;
  }
};
...
TEST_CASE("Aligned buffer alignment and size") {
  ...
  STATIC_CHECK(alignof(AlignedTypeBuffer<S>) == alignof(S));
