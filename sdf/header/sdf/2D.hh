...
float sdRoundedBox(const fvec2 p, fvec2 b, fvec4 r) noexcept {
  r("xy"_swz) = (p.x() > 0.f) ? fvec2(r("xy"_swz)) : fvec2(r("zw"_swz));
  r.x()       = (p.y() > 0.f) ? r.x() : r.y();

  const fvec2 q = abs(p) - b + r.x();
  return min(max(q.x(), q.y()), 0.f) + length(max(q, 0.f)) - r.x();
}

// Chamfer Box - exact (https://www.shadertoy.com/view/3fc3zs)
constexpr
float sdChamferBox(fvec2 p, const fvec2 b, const float chamfer) noexcept {
  p = abs(p) - b;
  p = (p.y() > p.x()) ? fvec2(p("yx"_swz)) : fvec2(p("xy"_swz));
  p.y() += chamfer;

  const float k = 1.0f - sqrt(2.0f);
  if (p.y() < 0.f && p.y() + p.x() * k < 0.f) {
    return p.x();
  }
...
constexpr
float sdBox(const fvec2 p, const fvec2 b) noexcept {
...
constexpr
float sdSegment(const fvec2 p, const fvec2 a, const fvec2 b) noexcept {
  const fvec2 pa = p - a, ba = b - a;
  const float h = clamp(dot(pa, ba) / dot(ba, ba), 0.f, 1.0f);
  return length(pa - ba * h);
}

// Rhombus - exact   (https://www.shadertoy.com/view/XdXcRB)
constexpr
float sdRhombus(fvec2 p, const fvec2 b) noexcept {
  p             = abs(p);
...
constexpr
float sdTrapezoid(fvec2 p, const float r1, const float r2, const float he) noexcept {
  const fvec2 k1 = fvec2(r2, he);
  const fvec2 k2 = fvec2(r2 - r1, 2.0f * he);
  p.x()          = abs(p.x());
  const fvec2 ca = fvec2(p.x() - min(p.x(), (p.y() < 0.f) ? r1 : r2), abs(p.y()) - he);
  const fvec2 cb = p - k1 + k2 * clamp(dot(k1 - p, k2) / dot2(k2), 0.f, 1.0f);
  const float s  = (cb.x() < 0.f && ca.y() < 0.f) ? -1.0f : 1.0f;
  return s * sqrt(min(dot2(ca), dot2(cb)));
}

// Parallelogram - exact   (https://www.shadertoy.com/view/7dlGRf)
constexpr
float sdParallelogram(fvec2 p, float wi, const float he, const float sk) noexcept {
...
constexpr
float sdEquilateralTriangle(fvec2 p, const float r) noexcept {
  ...
  p.x() -= clamp(p.x(), -2.0f * r, 0.f);
  return -length(p) * sign(p.y());
}

// Isosceles Triangle - exact   (https://www.shadertoy.com/view/MldcD7)
constexpr
float sdTriangleIsosceles(fvec2 p, const fvec2 q) noexcept {
  p.x()   = abs(p.x());
  fvec2 a = p - q * clamp(dot(p, q) / dot(q, q), 0.f, 1.0f);
  fvec2 b = p - q * fvec2(clamp(p.x() / q.x(), 0.f, 1.0f), 1.0f);
  float s = -sign(q.y());
  fvec2 d = min(fvec2(dot(a, a), s * (p.x() * q.y() - p.y() * q.x())), fvec2(dot(b, b), s * (p.y() - q.y())));
  return -sqrt(d.x()) * sign(d.y());
}

// Triangle - exact   (https://www.shadertoy.com/view/XsXSz4)
constexpr
float sdTriangle(const fvec2 p, const fvec2 p0, const fvec2 p1, const fvec2 p2) noexcept {
  const fvec2 e0 = p1 - p0, e1 = p2 - p1, e2 = p0 - p2;
  const fvec2 v0 = p - p0, v1 = p - p1, v2 = p - p2;
  const fvec2 pq0 = v0 - e0 * clamp(dot(v0, e0) / dot(e0, e0), 0.f, 1.0f);
  const fvec2 pq1 = v1 - e1 * clamp(dot(v1, e1) / dot(e1, e1), 0.f, 1.0f);
  const fvec2 pq2 = v2 - e2 * clamp(dot(v2, e2) / dot(e2, e2), 0.f, 1.0f);
...
constexpr
float sdUnevenCapsule(fvec2 p, const float r1, const float r2, const float h) noexcept {
  p.x()         = abs(p.x());
  ...
  if (k > a * h) {
    return length(p - fvec2(0.f, h)) - r2;
  }
...
constexpr
float sdPentagon(fvec2 p, const float r) noexcept {
  const fvec3 k = fvec3(0.809016994f, 0.587785252f, 0.726542528f);
  p.x()         = abs(p.x());
  p -= 2.0f * min(dot(fvec2(-k.x(), k.y()), p), 0.f) * fvec2(-k.x(), k.y());
  p -= 2.0f * min(dot(fvec2(k.x(), k.y()), p), 0.f) * fvec2(k.x(), k.y());
...
constexpr
float sdHexagon(fvec2 p, const float r) noexcept {
  const fvec3 k = fvec3(-0.866025404f, 0.5f, 0.577350269f);
  p             = abs(p);
  p -= 2.0f * min(dot(k("xy"_swz), p), 0.f) * k("xy"_swz);
...
constexpr
float sdHexagram(fvec2 p, const float r) noexcept {
  const fvec4 k = fvec4(-0.5f, 0.8660254038f, 0.5773502692f, 1.7320508076f);
  p             = abs(p);
  p -= 2.0f * min(dot(k("xy"_swz), p), 0.f) * k("xy"_swz);
  p -= 2.0f * min(dot(k("yx"_swz), p), 0.f) * k("yx"_swz);
...
constexpr
float sdPentagram(fvec2 p, const float r) noexcept {
  ...
  p.x() = abs(p.x());
  p -= 2.0f * max(dot(v1, p), 0.f) * v1;
  p -= 2.0f * max(dot(v2, p), 0.f) * v2;
  p.x() = abs(p.x());
  p.y() -= r;

  return length(p - v3 * clamp(dot(p, v3), 0.f, k1z * r)) * sign(p.y() * v3.x() - p.x() * v3.y());
}

// Regular Star - exact   (https://www.shadertoy.com/view/3tSGDy)
constexpr
float sdStar(fvec2 p, const float r, const int n, const float m) noexcept {
...
constexpr
float sdPie(fvec2 p, const fvec2 c, const float r) noexcept {
  p.x()         = abs(p.x());
  const float l = length(p) - r;
  const float m = length(p - c * clamp(dot(p, c), 0.f, r)); // c=sinf / cosf of aperture
...
constexpr
float sdCutDisk(fvec2 p, const float r, const float h) noexcept {
  const float w = sqrt(r * r - h * h); // constant for any given shape

  p.x()         = abs(p.x());
  const float s = max((h - r) * p.x() * p.x() + w * w * (h + r - 2.0f * p.y()), h * p.x() - w * p.y());

  return (s < 0.f) ? length(p) - r : (p.x() < w) ? h - p.y() : length(p - fvec2(w, h));
}

// Arc - exact   (https://www.shadertoy.com/view/wl23RK)
constexpr
float sdArc(fvec2 p, const fvec2 sc, const float ra, const float rb) noexcept {
...
constexpr
float sdRing(fvec2 p, const fvec2 n, const float r, const float th) noexcept {
  p.x() = abs(p.x());
  p     = mat2x2(n.x(), n.y(), -n.y(), n.x()) * p;
  return max(abs(length(p) - r) - th * 0.5f, length(fvec2(p.x(), max(0.f, abs(r - p.y()) - th * 0.5f))) * sign(p.x()));
}

// Horseshoe - exact   (https://www.shadertoy.com/view/WlSGW1)
constexpr
float sdHorseshoe(fvec2 p, const fvec2 c, const float r, const fvec2 w) noexcept {
  p.x()         = abs(p.x());
  const float l = length(p);
  p             = mat2x2(-c.x(), c.y(), c.y(), c.x()) * p;
  p             = fvec2((p.y() > 0.f || p.x() > 0.f) ? p.x() : l * sign(-c.x()), (p.x() > 0.f) ? p.y() : l);
...
constexpr
float sdVesica(fvec2 p, const float w, const float h) noexcept {
  const float d = 0.5f * (w * w - h * h) / h;
  p             = abs(p);
  const fvec3 c = (w * p.y() < d * (p.x() - w)) ? fvec3(0.f, w, 0.f) : fvec3(-d, 0.f, d + h);
  return length(p - c("yx"_swz)) - c.z();
}

// Oriented Vesica - exact   (https://www.shadertoy.com/view/cs2yzG)
constexpr
float sdOrientedVesica(const fvec2 p, const fvec2 a, const fvec2 b, const float w) noexcept {
  ...
  const fvec3 h = (r * q.x() < d * (q.y() - r)) ? fvec3(0.f, r, 0.f) : fvec3(-d, 0.f, d + w);
  return length(q - h("xy"_swz)) - h.z();
}

// Moon - exact   (https://www.shadertoy.com/view/WtdBRS)
constexpr
float sdMoon(fvec2 p, const float d, const float ra, const float rb) noexcept {
  p.y()   = abs(p.y());
  ...
  return max((length(p) - ra), -(length(p - fvec2(d, 0.f)) - rb));
}

// Circle Cross - exact   (https://www.shadertoy.com/view/NslXDM)
constexpr
float sdRoundedCross(fvec2 p, const float h) noexcept {
  float k = 0.5f * (h + 1.f / h);

  p = abs(p);

  return (p.x() < 1.0f && p.y() < p.x() * (k - h) + h) ? k - sqrt(dot2(p - fvec2(1.f, k)))
                                                       : sqrt(min(dot2(p - fvec2(0.f, h)), dot2(p - fvec2(1.f, 0.f))));
}

// Simple Egg - exact   (https://www.shadertoy.com/view/XtVfRW)
constexpr
float sdEgg(fvec2 p, const float ra, const float rb) noexcept {
...
float sdHeart(fvec2 p) noexcept {
  ...
  return sqrt(min(dot2(p - fvec2(0.00f, 1.00f)), dot2(p - .5f * max(p.x() + p.y(), 0.f)))) * sign(p.x() - p.y());
}

// Cross - exact exterior, bound interior   (https://www.shadertoy.com/view/XtGfzw())
constexpr
float sdCross(fvec2 p, const fvec2 b, const float r) noexcept {
  p = abs(p);
  p = (p.y() > p.x()) ? fvec2(p("yx"_swz)) : fvec2(p("xy"_swz));

  const fvec2 q = p - b;
  const float k = max(q.y(), q.x());
  const fvec2 w = (k > 0.f) ? q : fvec2(b.y() - p.x(), -k);
...
constexpr
float sdRoundedX(fvec2 p, const float w, const float r) noexcept {
  p = abs(p);
...
constexpr
float sdPolygon(const std::vector<fvec2>& v, const fvec2 p) noexcept {
  const auto N = v.size();
  ...
  for (int i = 0, j = N - 1; i < N; j = i, i++) {
    const fvec2 e = v[j] - v[i];
    const fvec2 w = p - v[i];
    const fvec2 b = w - e * clamp(dot(w, e) / dot(e, e), 0.f, 1.0f);
    d             = min(d, dot(b, b));
    const bvec3 c = bvec3(p.y() >= v[i].y(), p.y() < v[j].y(), e.x() * w.y() > e.y() * w.x());
...
constexpr
float sdParabola(fvec2 pos, const float k) noexcept {
  pos.x() = abs(pos.x());
  ...
  const float x  = (h > 0.f) ? pow(q + r, 1.0f / 3.0f) - pow(abs(q - r), 1.0f / 3.0f) * sign(r - q)
...
constexpr
float sdParabola(fvec2 pos, const float wi, const float he) noexcept {
  pos.x() = abs(pos.x());

  const float ik = wi * wi / he;
  const float p  = ik * (he - pos.y() - 0.5f * ik) / 3.0f;
  const float q  = pos.x() * ik * ik * 0.25f;
  const float h  = q * q - p * p * p;
  const float r  = sqrt(abs(h));
  float       x  = (h > 0.f) ? pow(q + r, 1.0f / 3.0f) - pow(abs(q - r), 1.0f / 3.0f) * sign(r - q)
                             : 2.0f * cosf(atanf(r / q) / 3.0f) * sqrt(p);

  x = min(x, wi);

  return length(pos - fvec2(x, he - x * x / ik)) * sign(ik * (pos.y() - he) + pos.x() * pos.x());
}

// Quadratic Bezier - exact   (https://www.shadertoy.com/view/MlKcDD)
constexpr
float sdBezier(fvec2 pos, const fvec2 A, const fvec2 B, const fvec2 C) noexcept {
  const fvec2 a  = B - A;
  ...
  if (h >= 0.f) {
    ...
    const float t  = clamp(uv.x() + uv.y() - kx, 0.f, 1.0f);
    ...
    const fvec3 t = clamp(fvec3(m + m, -n - m, n - m) * z - kx, 0.f, 1.0f);
...
constexpr
float sdBlobbyCross(fvec2 pos, const float he) noexcept {
  pos = abs(pos);
...
constexpr
float sdTunnel(fvec2 p, const fvec2 wh) noexcept {
  p.x() = abs(p.x());
  p.y() = -p.y();

  fvec2       q  = p - wh;
  const float d1 = dot2(fvec2(max(q.x(), 0.f), q.y()));

  q.x() = (p.y() > 0.f) ? q.x() : length(p) - wh.x();

  const float d2 = dot2(fvec2(q.x(), max(q.y(), 0.f)));
...
constexpr
float sdStairs(fvec2 p, const fvec2 wh, const float n) noexcept {
  const fvec2 ba = wh * n;
  float d   = min(dot2(p - fvec2(clamp(p.x(), 0.f, ba.x()), 0.f)), dot2(p - fvec2(ba.x(), clamp(p.y(), 0.f, ba.y()))));
  float s   = sign(max(-p.y(), p.x() - ba.x()));
  float dia = length(wh);

  p = mat2x2(wh.x(), -wh.y(), wh.y(), wh.x()) * p / dia;

  const float id = clamp(round(p.x() / dia), 0.f, n - 1.0f);
  ...
  d = min(d, dot2(p - fvec2(0.f, clamp(p.y(), -hh, hh))));
  d = min(d, dot2(p - fvec2(clamp(p.x(), 0.f, wh.x()), hh)));
...
float sdQuadraticCircle(fvec2 p) noexcept {
  ...
  const float a = p.x() - p.y();
  const float b = p.x() + p.y();
  const float c = (2.0f * b - 1.0f) / 3.0f;
  ...
  if (h >= 0.f) {
    ...
    const float z = sqrt(-c);
    const float v = acosf(a / (c * z)) / 3.0f;
...
// k (0, inf)
constexpr
float sdHyperbola(fvec2 p, const float k, const float he) noexcept {
  p = abs(p);
  p = fvec2(p.x() - p.y(), p.x() + p.y()) / sqrt(2.0f);

  const float x2 = p.x() * p.x() / 16.0f;
  const float y2 = p.y() * p.y() / 16.0f;
  const float r  = k * (4.0f * k - p.x() * p.y()) / 12.0f;
  const float q  = (x2 - y2) * k * k;
  const float h  = q * q + r * r * r;

  float u;
  if (h < 0.f) {
    const float m = sqrt(-r);
    u             = m * cosf(acosf(q / (r * m)) / 3.0f);
  } else {
    const float m = pow(sqrt(h) - q, 1.0f / 3.0f);
    u             = (m - r / m) / 2.0f;
  }

  const float w = sqrt(u + x2);
  const float b = k * p.y() - x2 * p.x() * 2.0f;
  float       t = p.x() / 4.0f - w + sqrt(2.0f * x2 - u + b / w / 4.0f);

  t = max(t, sqrt(he * he * 0.5f + k) - he / sqrt(2.0f));

  const float d = length(p - fvec2(t, k / t));
...
float sdfCoolS(fvec2 p) noexcept {
  float six = (p.y() < 0.f) ? -p.x() : p.x();
  p.x()     = abs(p.x());
  p.y()     = abs(p.y()) - 0.2f;
  float rex = p.x() - min(round(p.x() / 0.4f), 0.4f);
  float aby = abs(p.y() - 0.2f) - 0.6f;

  float d = dot2(fvec2(six, -p.y()) - clamp(0.5f * (six - p.y()), 0.f, 0.2f));
  d       = min(d, dot2(fvec2(p.x(), -aby) - clamp(0.5f * (p.x() - aby), 0.f, 0.4f)));
  d       = min(d, dot2(fvec2(rex, p.y() - clamp(p.y(), 0.f, 0.4f))));
