...
constexpr
float sdCircle(const vec2 p, float r) noexcept {
  return length(p) - r;
}
...
constexpr
float sdBox(const vec2 p, vec2 b) noexcept {
...
constexpr
float sdSegment(const vec2 p, vec2 a, vec2 b) noexcept {
...
float sdRhombus(vec2 p, vec2 b) noexcept {
  p = abs(p);
  const float h = clamp(ndot(b - 2.0 * p, b) / dot(b, b), -1.0, 1.0);
  const float d = length(p - 0.5 * b * vec2(1.0 - h, 1.0 + h));
...
float sdTrapezoid(vec2 p, float r1, float r2, float he) noexcept {
  const vec2 k1 = vec2(r2, he);
  const vec2 k2 = vec2(r2 - r1, 2.0 * he);
  p.x = abs(p.x);
  const vec2 ca = vec2(p.x - min(p.x, (p.y<0.0) ? r1 : r2), abs(p.y) - he);
  const vec2 cb = p - k1 + k2 * clamp(dot(k1 - p, k2) / dot2(k2), 0.0, 1.0);
...
float sdEllipse(vec2 p, vec2 ab) noexcept {
  ...
  const float n2 = n * n;
  const float c = (m2 + n2 - 1.0) / 3.0;
  const float c3 = c * c * c;
  const float q = c3 + m2 * n2 * 2.0;
  const float d = c3 + m2 * n2;
  const float g = m + m * n2;
  float co;

  if (d<0.0) {
    const float h = acos(q / c3) / 3.0;
    const float s = cos(h);
    const float t = sin(h) * sqrt(3.0);
    const float rx = sqrt(-c * (s + t + 2.0) + m2);
    const float ry = sqrt(-c * (s - t + 2.0) + m2);
    co = (ry + sign(l) * rx + abs(g) / (rx * ry)- m) / 2.0;
  } else {
    const float h = 2.0 * m * n * sqrt(d);
    const float s = sign(q + h) * pow(abs(q + h), 1.0 / 3.0);
    const float u = sign(q - h) * pow(abs(q - h), 1.0 / 3.0);
    const float rx = -s - u - c * 4.0 + 2.0 * m2;
    const float ry = (s - u) * sqrt(3.0);
    const float rm = sqrt(rx * rx + ry * ry);
    co = (ry / sqrt(rm - rx) + 2.0 * g / rm - m) / 2.0;
  }
  const vec2 r = ab * vec2(co, sqrt(1.0 - co * co));
...
float sdParabola(vec2 pos, float k) noexcept {
  ...
  const float p = ik * (pos.y - 0.5 * ik) / 3.0;
  const float q = 0.25 * ik * ik * pos.x;
...
float sdBezier(vec2 pos, vec2 A, vec2 B, vec2 C) noexcept {
  ...
  if (h > = 0.0) {
    h = sqrt(h);
    const vec2 x = (vec2(h, -h) - q) / 2.0;
    const vec2 uv = sign(x) * pow(abs(x), vec2(1.0 / 3.0));
    const float t = clamp(uv.x + uv.y - kx, 0.0, 1.0);
    ...
    const vec3  t = clamp(vec3(m + m, -n-m, n - m) * z - kx, 0.0, 1.0);
...
float sdBlobbyCross(vec2 pos, float he) noexcept {
  ...
  const float p = (he - pos.y - 0.25 / he) / (6.0 * he);
  const float q = pos.x / (he * he * 16.0);
  const float h = q * q - p * p * p;
  ...
  const vec2 z = vec2(x, he * (1.0 - 2.0 * x * x)) - pos;
...
float sdTunnel(vec2 p, vec2 wh) noexcept {
  ...
  const float d1 = dot2(vec2(max(q.x, 0.0), q.y));
  q.x = (p.y > 0.0) ? q.x : length(p) - wh.x;
  const float d2 = dot2(vec2(q.x, max(q.y, 0.0)));
