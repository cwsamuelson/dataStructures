...
float sdRing(fvec2 p, const fvec2 n, const float r, const float th) noexcept {
  ...
  return max(abs(length(p) - r) - th * 0.5f,
              length(fvec2(p.x(), max(0.f, abs(r - p.y()) - th * 0.5f))) * sign(p.x()));
}
...
float sdHorseshoe(fvec2 p, const fvec2 c, const float r, const fvec2 w) noexcept {
  ...
  p = fvec2((p.y() > 0.f || p.x() > 0.f) ? p.x() : l * sign(-c.x()),
           (p.x() > 0.f) ? p.y() : l);
...
float sdStairs(fvec2 p, const fvec2 wh, const float n) noexcept {
  const fvec2 ba = wh * n;
  float d = min(dot2(p - fvec2(clamp(p.x(), 0.f, ba.x()), 0.f)),
                dot2(p - fvec2(ba.x(), clamp(p.y(), 0.f, ba.y()))));
