...
TEST_CASE("Mathematical operators behave as expected", "[equ]") {
  ...
  SECTION("Multiplication (*)") {
    CHECK(("X"_evar * "Y"_evar).evaluate({{ "X", 2 },
                                          { "Y", 3 }}) == 6);
    CHECK(("X"_evar * "Y"_evar).evaluate({{ "X", 3 },
                                          { "Y", -3 }}) == -9);
    CHECK(("X"_evar * "Y"_evar).evaluate({{ "X", -4 },
                                          { "Y", -3 }}) == 12);
  ...
  SECTION("Division (/)") {
    CHECK(("X"_evar / "Y"_evar).evaluate({{ "X", 12 },
                                          { "Y", 4 }}) == 3);
    CHECK(("X"_evar / "Y"_evar).evaluate({{ "X", 12 },
                                          { "Y", -2 }}) == -6);
  ...
  SECTION("Addition (+)") {
    CHECK(("X"_evar + "Y"_evar).evaluate({{ "X", 1 },
                                          { "Y", 2 }}) == 3);
    CHECK(("X"_evar + "Y"_evar).evaluate({{ "X", 3 },
                                          { "Y", 2 }}) == 5);
    CHECK(("X"_evar + "Y"_evar).evaluate({{ "X", 5 },
                                          { "Y", -7 }}) == -2);
