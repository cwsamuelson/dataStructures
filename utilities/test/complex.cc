...
TEST_CASE("`Complex`::`float`") {
  ...
  rc::prop("Math with 'normal' numbers", [](const float r, const float i) {
    const auto f = *rc::gen::nonZero<float>();
    const Complex<float> complex{r, i};

    RC_ASSERT(complex + f == (Complex<float>{r + f, i}));
    RC_ASSERT(f + complex == (Complex<float>{f + r, i}));
    RC_ASSERT(complex - f == (Complex<float>{r - f, i}));
    RC_ASSERT((f - complex) == (Complex<float>{f - r, -i}));

    RC_ASSERT(complex * f == (Complex<float>{r * f, i * f}));
    RC_ASSERT(f * complex == (Complex<float>{f * r, f * i}));
    RC_ASSERT(complex / f == (Complex<float>{r / f, i / f}));
...
// conceptually do the same tests as above, but using one of my 'ranged' types.
// Presumably a floating point one, not the integral one I have now, but really
// either should work.
TEST_CASE("`Complex`::`RangedNumber`") {
  rc::prop("Initialization", [](const float r, const float i) {
    const Complex<float> value{r, i};
