...
TEST_CASE("Ranges can filter values from a container", "[range]") {
  ...
  gsw::range<decltype(source)> rng(source, [](decltype( source )::value_type x) -> bool
    {
      return x % 2 == 0;
    });

  std::generate(source.begin(), source.end(), []() -> int
    {
      static int i = 0;
      return i++;
    });
  std::generate(result.begin(), result.end(), []() -> int
    {
      static int i = -2;
...
TEST_CASE("Ranges can modify the values from a container before returning them", "[range]") {
  ...
  gsw::range<decltype(source)> rng(source, [](decltype( source )::value_type)
    {
      return true;
