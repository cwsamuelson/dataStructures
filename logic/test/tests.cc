...
TEST_CASE( "Logic operators behave as expected", "[logic]" ){
  ...
  SECTION( "Basic operation" ){
    ...
    CHECK( ( foo ).solve2( {"foo"} ) ==
            set<set<string> >{{"foo"}} );

    CHECK( ( foo ).solve2( {"foo"} ) ==
            set<set<string> >{{"foo"}} );

    CHECK( ( foo ).solve2( {"bar"} ) ==
            set<set<string> >{{"bar", "foo"}, {"foo"}} );
  }
  ...
  SECTION( "Implication" ){
    ...
    CHECK( ( foo.implies( bar ) ).solve( {"foo", "bar"} ) ==
             set<set<string> >{{"foo", "bar"}, {"bar"}, {}} );

    CHECK( ( foo.implies( foo ) ).solve( {"foo"} ) ==
             set<set<string> >{{"foo"}, {}} );
    ...
    CHECK( ( foo.implies( bar ) ).solve2( {"foo", "bar"} ) ==
            set<set<string> >{{"foo", "bar"}, {"bar"}, {}} );

    CHECK( ( foo.implies( foo ) ).solve2( {"foo"} ) ==
            set<set<string> >{{"foo"}, {}} );
  ...
  SECTION( "Tautology (equivalence, iff)" ){
    ...
    // foo and bar both false is a valid answer, so empty set is included
    CHECK( ( foo.iff( bar ) ).solve( {"foo", "bar"} ) ==
             set<set<string> >{{"foo", "bar"}, {}} );

    CHECK( ( foo.iff( bar ) ).solve2( {"foo", "bar"} ) ==
            set<set<string> >{{"foo", "bar"}, {}} );
  }
  ...
  SECTION( "Can solve compound propositions" ){
    CHECK( ( ( foo || bar ) && ( baz || qux ) ).solve( {"foo", "bar", "baz", "qux"} ) ==
             set<set<string> >{{"bar", "qux"}, {"foo", "qux"}, {"foo", "bar", "qux"},
                                 {"bar", "baz"}, {"bar", "baz", "qux"}, {"foo", "baz"},
                                 {"foo", "baz", "qux"}, {"foo", "bar", "baz"},
                                 {"foo", "bar", "baz", "qux"}} );

    CHECK( ( ( foo && bar ) || ( baz && qux ) ).solve( {"foo", "bar", "baz", "qux"} ) == set<set<string> >{{"foo", "bar"}, {"baz", "qux"}, {"foo", "bar", "baz"}, {"foo", "bar", "qux"}, {"baz", "qux", "foo"}, {"baz", "qux", "bar"}, {"foo", "bar", "baz", "qux"}} );


    CHECK( ( ( foo || bar ) && ( baz || qux ) ).solve2( {"foo", "bar", "baz", "qux"} ) ==
             set<set<string> >{{"bar", "qux"}, {"foo", "qux"}, {"foo", "bar", "qux"},
                                 {"bar", "baz"}, {"bar", "baz", "qux"}, {"foo", "baz"},
                                 {"foo", "baz", "qux"}, {"foo", "bar", "baz"},
                                 {"foo", "bar", "baz", "qux"}} );

    CHECK( ( ( foo && bar ) || ( baz && qux ) ).solve2( {"foo", "bar", "baz", "qux"} ) == set<set<string> >{{"foo", "bar"}, {"baz", "qux"}, {"foo", "bar", "baz"}, {"foo", "bar", "qux"}, {"baz", "qux", "foo"}, {"baz", "qux", "bar"}, {"foo", "bar", "baz", "qux"}} );
  }
}
