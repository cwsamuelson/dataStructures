...
TEST_CASE("variables", "[lambda]") {
  struct X {};
  struct Y {};
  using env1 = binding<Y, succeed<Zero, 12>, binding<X, succeed<Zero, 5>>>;
...
TEST_CASE("conditionals", "[lambda]") {
  ...
  using E1 = eval<eval<If<literal<True>, ref<X>, ref<Y>>, empty_env>::result, env1>::result;
  using E2 = eval<eval<If<literal<False>, ref<X>, ref<Y>>, empty_env>::result, env1>::result;

  CHECK(E1::value == 5);
  CHECK(E2::value == 12);

  using E3 = eval<eval<If<literal<True>, ref<X>, ref<Y>>, empty_env>::result, env2>::result;
  using E4 = eval<eval<If<literal<False>, ref<X>, ref<Y>>, empty_env>::result, env2>::result;
