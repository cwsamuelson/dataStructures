...
template<typename T>
typename gsw::enable_if<!gsw::is_void<T>::value, int>::type fortyTwo() {
  return 0;
}
...
TEST_CASE("remove_cv", "[traits]") {
  ...
  CHECK(gsw::is_same<gsw::remove_extent<int* [12]>::type, int**>::value);
  CHECK(gsw::is_same<gsw::remove_pointer<gsw::remove_pointer<gsw::remove_extent<int* [12]>::type>::type>::type,
                int > ::value);
}
