...
public:
  ...
  bound
  operator+( const bound& rhs ) const{
    bound b;
...
public:
  ...
  void
  recur( location low, location half ){
...
public:
  ...
  segment_tree&
  operator=( const segment_tree& ) = default;
  ...
  auto
  size() const->decltype( mData.size() ){
...
template<typename T>
using double_tree = segment_tree<T, 1>;
template<typename T>
using quad_tree = segment_tree<T, 2>;
template<typename T>
using octree = segment_tree<T, 3>;
template<typename T>
using hexadecatree = segment_tree<T, 4>;
