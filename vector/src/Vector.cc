...
template<class T>
Jing::Vector<T>::Vector(Collection<T>& c):Vector(){
  this->insertAll(c);
}

template<class T>
void Jing::Vector<T>::insert(T& obj){
  this->insert(obj, this->size());
}

//TODO:Optimize by inserting from back using negative indexes
template<class T>
void Jing::Vector<T>::insert(T& obj, Jing::index_t idx){
  //uninitialized list
  if(this->size() == 0){
  //  update first
...
template<class T>
void Jing::Vector<T>::insertAll(Collection<T>& c){
  this->insertAll(c, this->size() - 1);
}

template<class T>
void Jing::Vector<T>::insertAll(Collection<T>& c, Jing::index_t idx){
...
template<class T>
T& Jing::Vector<T>::remove(){
  return this->remove((Jing::index_t)0);
}

template<class T>
T& Jing::Vector<T>::remove(Jing::index_t idx){
...
template<class T>
void Jing::Vector<T>::remove(T& obj){
...
template<class T>
void Jing::Vector<T>::removeAll(Collection<T>& c){
  Iterator<T>& iter = c.iterator();
  while(iter.hasNext()){
...
template<class T>
void Jing::Vector<T>::removeRange(Jing::index_t fromIndex, Jing::index_t toIndex){
...
template<class T>
T& Jing::Vector<T>::get() const{
  return this->get(0);
}

template<class T>
T& Jing::Vector<T>::get(Jing::index_t idx) const{
  //just return last
  if(idx >= this->size()){
...
template<class T>
Jing::AbstractList<T>& Jing::Vector<T>::get(Jing::index_t start, Jing::index_t end) const{
  List& ret = *new Vector();
...
template<class T>
T& Jing::Vector<T>::assign(Jing::index_t idx, T& obj){
...
template<class T>
bool Jing::Vector<T>::contains(T& obj) const{
  if(this->size() > 0){
...
template<class T>
bool Jing::Vector<T>::containsAll(Collection<T>& c) const{
  Iterator<T>& iter = c.iterator();
  while(iter.hasNext()){
...
template<class T>
Jing::index_t Jing::Vector<T>::indexOf(T& obj) const{
  return this->first->find(obj, 0);
}

template<class T>
Jing::index_t Jing::Vector<T>::lastIndexOf(T& obj) const{
...
template<class T>
bool Jing::Vector<T>::equals(Jing::Object& obj) const{
...
template<class T>
Jing::Iterator<T>& Jing::Vector<T>::iterator() const{
  this->iter->reset();
  return *(this->iter);
}
...
*/
template<class T>
unsigned long long Jing::Vector<T>::hash() const{
  unsigned long len = 1;
...
template<class T>
Jing::Object* Jing::Vector<T>::clone() const{
  Jing::Vector<T>* temp = new Vector<T>(*this);
  return temp;
}

//create a new array containing all elements in proper order.
template<class T>
Jing::Object* Jing::Vector<T>::toArray() const{
  return 0;
}
...
template<class T>
void Jing::Vector<T>::toArray(Jing::Object* arr) const{
...
template<class T>
bool Jing::Vector<T>::isEmpty() const{
  if(this->size() == 0)
    return true;
  else
    return false;
...
template<class T>
Jing::size_t Jing::Vector<T>::size() const{
  return this->count;
}
...
template<class T>
Jing::Vector<T>::listNode::listNode(T& obj):
  data(obj),
  prev(0),
  next(0){  }

template<class T>
Jing::Vector<T>::listNode::listNode(T& obj, listNode* prev, listNode* next):
  data(obj),
  prev(prev),
  next(next){ }

template<class T>
bool Jing::Vector<T>::listNode::add(T& obj, Jing::index_t idx){
  if(idx > 0){
...
template<class T>
T& Jing::Vector<T>::listNode::remove(Jing::index_t idx){
  if(idx > 0){
...
template<class T>
T& Jing::Vector<T>::listNode::get(Jing::index_t idx) const{
  if(idx > 0){
    if(this->next != 0){
...
template<class T>
bool Jing::Vector<T>::listNode::assign(Jing::index_t idx, T& obj){
  bool ret = false;
  if(idx > 0){
...
template<class T>
Jing::index_t Jing::Vector<T>::listNode::find(T& obj, Jing::index_t idx){
...
template<class T>
Jing::index_t Jing::Vector<T>::listNode::backFind(T& obj, Jing::index_t idx){
  if(!this->data.equals(obj)){
    if(this->prev != 0){
...
template<class T>
Jing::Vector<T>::listIterator::listIterator(Jing::Vector<T>& thisList):listIterator(thisList, 0){
}

template<class T>
Jing::Vector<T>::listIterator::listIterator(Jing::Vector<T>& thisList, Jing::index_t idx):idx(idx), theList(thisList){
}

template<class T>
bool Jing::Vector<T>::listIterator::hasNext(){
...
template<class T>
T& Jing::Vector<T>::listIterator::next(){
  return this->theList.get(this->idx++);
}

template<class T>
void Jing::Vector<T>::listIterator::remove(){
  this->theList.remove(idx);
}

template<class T>
void Jing::Vector<T>::listIterator::reset(){
  this->idx = 0;
}
