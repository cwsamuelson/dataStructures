...
struct OctTree {
  ...
  struct AABB {
    ...
    bool contains(const Point& point) const noexcept {
      return point.x >= corner.x
         ...
         and point.z >= corner.z
         and point.z < corner.z + dimensions.z;
    }
    ...
    bool intersects(const AABB& other) const noexcept {
      ...
      return ((other.corner.x <= (corner.x + dimensions.x))
         ...
         and ((other.corner.z <= (corner.z + dimensions.z))
         and  (corner.z <= (other.corner.z + other.dimensions.z)));
    }
  ...
  using Children = std::array<Node, 8>;
